...
JNIEXPORT jint JNICALL Java_com_scriptographer_ai_MenuGroup_nativeCreate(JNIEnv *env, jobject obj, jstring name, jstring nearGroup, jint parentItemHandle, jint options) {
	try {
		...
		if (nearGroup != NULL) {
			char *nearGroupStr = gEngine->createCString(env, nearGroup);
 ...
 */
JNIEXPORT void JNICALL Java_com_scriptographer_ai_MenuGroup_setOption(JNIEnv *env, jobject obj, jint options) {
	try {
 ...
 * int getOptions()
 */
JNIEXPORT jint JNICALL Java_com_scriptographer_ai_MenuGroup_getOptions(JNIEnv *env, jobject obj) {
	try {
		AIMenuGroup group = gEngine->getMenuGroupHandle(env, obj);
		long options = 0;
