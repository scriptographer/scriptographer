...
#include "com_scriptographer_adm_MenuGroup.h"
 ...
 */
JNIEXPORT jint JNICALL Java_com_scriptographer_adm_MenuGroup_nativeCreate(JNIEnv *env, jclass cls, jstring name, jstring nearGroup, jint parentItemHandle, jint options) {
	try {
 ...
 * int getOptions()
 */
JNIEXPORT jint JNICALL Java_com_scriptographer_adm_MenuGroup_getOptions(JNIEnv *env, jobject obj) {
	try {
		AIMenuGroup group = gEngine->getMenuGroupHandle(env, obj);
		long options = 0;
