...
char *ScriptographerEngine::reloadEngine() {
	...
	initEngine();
	if (errors != NULL) {
		return createCString(env, errors);
	} else {
		println(env, "The classpath was successfully reloaded.");
		return NULL;
	}
}
...
// ADM:
	...
	mid_Dialog_onCreate = getMethodID(env, cls_Dialog, "onCreate", "()V");
	...
	cid_List = getConstructorID(env, cls_List, "(I)V");
	
	cls_HierarchyList = loadClass(env, "com/scriptographer/adm/HierarchyList");
	cid_HierarchyList = getConstructorID(env, cls_HierarchyList, "(I)V");
	...
	cid_ListEntry = getConstructorID(env, cls_ListEntry, "(I)V");

	cls_HierarchyListEntry = loadClass(env, "com/scriptographer/adm/HierarchyListEntry");
	cid_HierarchyListEntry = getConstructorID(env, cls_HierarchyListEntry, "(I)V");
	...
	mid_CallbackHandler_onDestroy = getMethodID(env, cls_CallbackHandler, "onDestroy", "(Lcom/scriptographer/adm/ListEntry;)V");
	...
	mid_CallbackHandler_onDraw = getMethodID(env, cls_CallbackHandler, "onDraw", "(Lcom/scriptographer/adm/Drawer;Lcom/scriptographer/adm/ListEntry;)V");
...
void ScriptographerEngine::callOnDraw(jobject listener, ADMDrawerRef drawer, jobject entry) {
	...
	try {
		...
		callVoidMethod(env, listener, mid_CallbackHandler_onDraw, drawerObj, entry);
	} EXCEPTION_CATCH_REPORT(env)
}
 ...
 * throws exceptions
 */
ADMEntryRef ScriptographerEngine::getEntryRef(JNIEnv *env, jobject obj) {
	JNI_CHECK_ENV
...
jobject ScriptographerEngine::getEntryObject(ADMEntryRef entry) {
	jobject obj = NULL;
	if (sADMEntry != NULL) {
...
jobject ScriptographerEngine::getEntryObject(ADMListEntryRef entry) {
	jobject obj = NULL;
